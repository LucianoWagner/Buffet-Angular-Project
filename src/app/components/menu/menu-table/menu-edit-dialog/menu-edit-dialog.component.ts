...
export class MenuEditDialogComponent implements OnInit {
  ...
  public readonly context =
    injectContext<TuiDialogContext<Menu | null, Menu | null>>();
  ...
  protected readonly form = new FormGroup({
    ...
    price: new FormControl(this.context.data.price, [
      Validators.required,
      Validators.min(0),
    ]),
    stock: new FormControl(this.context.data.stock, [
      Validators.required,
      Validators.min(0),
    ]),
    starter: new FormControl(
      findComponent(this.context.data.components, 'STARTER')?.id,
    ),
    main_dish: new FormControl(
      findComponent(this.context.data.components, 'MAIN_DISH')?.id,
    ),
